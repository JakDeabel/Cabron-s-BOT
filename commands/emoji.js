...
exports.run = (bot, message, args) => {
    ...
    var emoji = bot.emojis.find(e => e.name == emojiName);
    ...
    if(!emoji){
        ...
        if(args[1].endsWith(".png") || args[1].endsWith(".jpg")){
            ...
            if(!text.startsWith("<img")){
                message.channel.send("Erro: Só funciona com emojis deste servidor / default emojis / png ou jpg urls.");
                return;
            }
    ...
    Jimp.read(url, (err, img) => {
        if(err){
            message.channel.send("Erro: Não consegui ler a imagem.");
            return;
        }
        img
            .resize(30, 30)
